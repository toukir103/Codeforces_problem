...
int main()
{
    ...
    while(t--){
        ...
        form(i,n){
            ...
            double lower_bound=static_cast<double>(i) / B;
            double upper_bound=static_cast<double>(i) / A;
            max_speed = max(max_speed, lower_bound);
            min_speed = min(min_speed, upper_bound);
        }
        cout << "Case #" << case_num << ": ";
        if (!possible || max_speed > min_speed) {cout << -1 << endl;}
         else {cout << fixed << setprecision(6) << max_speed << endl;}
        case_num++;
    }
    return 0;
}